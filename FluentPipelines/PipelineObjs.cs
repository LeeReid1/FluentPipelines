...
/// <summary>
/// A Pipeline only exposing its first and last elements that can be appended to another or have another appended to it. Indended only for use within <see cref="PipeExtensionMethods"/>
/// </summary>
...
/// <summary>
/// A Pipeline only exposing its first and last elements. Indended only for use within <see cref="PipeExtensionMethods"/>
/// </summary>
...
/// <summary>
/// A Pipeline only exposing its first element (which takes an input). Indended only for use within <see cref="PipeExtensionMethods"/>
/// </summary>
...
{
   //public static implicit operator Pipeline_RightSealed<TPipelineInput>(Func<T1, T3> func) => new Pipe<T1, T3>(func);

   //public static implicit operator Pipeline_RightSealed<TPipelineInput>(AsyncFunc<T1, T3> func) => new Pipe<T1, T3>(func);

   //public static implicit operator Pipeline_RightSealed<TPipelineInput>(Pipe<T1, T3> p) => new(p, p);
}
